/**
 * Created by ncampos on 6/20/14.
 */

var designer;

Template.designerTemplate.rendered = function() {
  //initialize the xr2designer with some optional options
  //var options = allBindingsAccessor().designerOptions || {};
  designer = xr2designer($('#content'), { baseUrl: Meteor.absoluteUrl() });
  window.designer = designer; // for testing

  // wait for the designer to be ready
  $('#content').on('xr2designer:loaded', function () {
    // load demo fields
    ...
    designer.setFieldsMetadata(auxData);
  });
};
