...
Template.taskLink.helpers({
    ...
    phone: function () {
        ...
        var phoneLookups = LookUps.find({
            lookUpCode: Enums.lookUpCodes.contactMethod_types,
            lookUpActions: {$in: [Enums.lookUpAction.ContactMethod_Phone]}
        }).fetch();
        ...
        _.each(contactable.contactMethods, function (c) {
            ...
            if (index > -1) {
                this.phoneRaw = c.value;
                toReturn = toReturn + "<br>" + phoneLookups[index].displayName + "<a href='" + c.value + "' " + "style='float:right'>" + c.value + "</a>";
                toReturn= c.value;
            }
        });
        return toReturn;
