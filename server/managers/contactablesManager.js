...
ContactableManager = {
  ...
  createFromCard: Meteor.wrapAsync(function (data, metadata, callback) {
    ...
    if (typeof data.read == 'function') {
      ...
      var progress = ServerProgress.start(user._id, 'processCard');
      progress.set(5);
      if (! user)
             ...
             if(task.status = "Completed"){
                ...
                HTTP.get(task.resultUrl, function(err, resultado){
                  ...
                  var objectR = xml2jsAsync(resultado.content);
                    ...
                    var addressTypeId = LookUps.findOne({lookUpCode:Enums.lookUpCodes.contactable_address,lookUpActions:Enums.lookUpAction.Address_WorksSite, hierId:user.currentHierId});
