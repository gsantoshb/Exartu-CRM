...
validateObjType = function (obj, objType) {
	...
	_.forEach(relations, function (rel) {
		var objRel = typeInfo.name == rel.obj1 ? rel.visibilityOn1 : rel.visibilityOn2;
		if (typeInfo[rel.name] != undefined) {
			v = v && beforeUpdateRelation(typeInfo[rel.name], objRel);
			if (!v) console.error(rel.name + ' is invalid: ' + v);
		} else {
			typeInfo[objRel.name] = objRel.defaultValue;
		}
	});
	if (!v) {
