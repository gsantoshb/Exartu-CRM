...
Meteor.publish('lastEntries', function (searchStringEntries, sort) {
  ...
  if(this.userId) {
    return [Utils.filterCollectionByUserHier.call(this, LastEntries.find({userId:this.userId, displayName:{$regex:".*"+searchStringEntries+".*",$options:'i'}},{sort:{pinged:-1,dateCreated:sort},limit:20}))];
  }
